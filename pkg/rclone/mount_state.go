...
func NewMountStateManager(nodeID string) (*MountStateManager, error) {
...
func (sm *MountStateManager) SaveState(ctx context.Context, state *MountState) error {
	...
	if err != nil {
		if errors.IsAlreadyExists(err) {
			existing, getErr := sm.secrets.Get(ctx, secret.Name, metav1.GetOptions{})
			if getErr != nil {
				return fmt.Errorf("failed to get existing state secret %s: %w", secret.Name, getErr)
			}
